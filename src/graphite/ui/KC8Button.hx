...
class KC8Button extends KC8Object
{
	private var label:TextField;
	private var background:Shape;
	private var _callback:KC8Button->Void;
	private var darken:Shape;
	private var over:Bool;
	
	public function new(_label:String = "default", _width:Int = 100, _height:Int = 20, __callback:KC8Button->Void = null) 
	{
		background = new Shape();
		background.graphics.lineStyle(1, KC8UI.configOptions.chrome_color_1);
		background.graphics.beginFill(KC8UI.configOptions.chrome_color_2);
		background.graphics.drawRect(0, 0, _width, _height);
		addChild(background);
		
		label = new TextField();
		label.defaultTextFormat = KC8UI.textFormat;
		label.text = _label;
